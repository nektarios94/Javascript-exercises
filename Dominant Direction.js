var SCRIPTS = [
  ...
  function dominantDirection (text) {
    ...
    textDirections = countBy (text,c => {
      let charDir = characterScript(c.codePointAt(0));
      if (!charDir) return null;
      return charDir.direction;
      }).filter(({name}) => name != null);
    // στη λύση μου δεν είχα σκεφτεί την περίπτωση ότι το δωθέν string μπορεί να είναι κενό
    // σε αυτην την περίπτωση το πρόγραμμα βγάζει error λόγω της reduce σε άδειο πίνακα
    
    // Αν το .name είναι έξω από την reduce, όπως στη λύση του βιβλίου τότε η παρακάτω if δε χριάζεται
    // Δεν έχω βρει γιατί 
    if (textDirections.length == 1)  return textDirections[0].name;
    return textDirections.reduce ((prev,curr) => prev.count >= curr.count? prev.name : curr.name);
  }
